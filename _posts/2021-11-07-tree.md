...
Binary tree that satisfies the condition of binary search tree
...
* Conditional processing
    ...
    3. Key value of Inserting node small than `p`
        ...
        * `p` node hasn't left child node
            insert node
    4. Key value of Inserting node large than `p`
